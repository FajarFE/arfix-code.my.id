...
import { fetchPostById,fetchPosts,setCreatedAt ,setData} from '../Store/PostsSlice';
...
export default function NextPage() {
  ...
  const {data, currentPost,createdAt, error,statusId } = useSelector((state) => state.posts);
  ...
  return (
    ...
    {currentPost && (
      ...
      <h2 className='capitalize text-[30px] my-2 font-bold break-words leading-none text-center w-full px-5'>{currentPost.title}</h2>
...
/>
        ...
        <div className="flex justify-center mt-4 absolute bottom-0 top-0 left-2">
          <button className='text-white ' onClick={prevSlide}>
            <svg className='w-[30px] h-[30px]  lg:h-[60px] lg:w-[60px]' fill="#16213E" viewBox="0 0 32 32" version="1.1" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="absolute bottom-2 right-0 left-0 flex justify-center align-items-center gap-5">
          <button
            ...
            className={`${currentImageIndex === 0 ? 'bg-[#16213E] lg:w-[60px] w-[20px] h-[8px] lg:h-[15px] rounded-[8px]' : '  border-2 lg:border-4 border-[#16213E] lg:w-[60px] w-[20px] h-[8px] lg:h-[15px] rounded-[8px]'}`}
          ></button>
          <button
            onClick={() => setCurrentImageIndex(1)}
            className={`${currentImageIndex === 1 ? 'bg-[#16213E] lg:w-[60px] w-[20px] h-[8px] lg:h-[15px] rounded-[8px]' : 'border-2 lg:border-4 border-[#16213E] lg:w-[60px] w-[20px] h-[8px] lg:h-[15px] rounded-[8px]'}`}
          ></button>
          <button
            onClick={() => setCurrentImageIndex(2)}
            className={`${currentImageIndex === 2 ? 'bg-[#16213E] lg:w-[60px] w-[20px] h-[8px] lg:h-[15px] rounded-[8px]' : ' border-2 lg:border-4 border-[#16213E] lg:w-[60px] w-[20px] h-[8px] lg:h-[15px] rounded-[8px]'}`}
          ></button>
        </div>
      </div>
...
<div className="grid grid-cols-12 gap-4 ">
      <div className="col-span-12 px-4 lg:px-0 md:px-0 lg:col-span-9 lg:my-10 md:px-0 flex w-full flex-col gap-4 ">
      <div className="break-words font-sans leading-none  w-full h-full "
     ...
     <div className="flex jutify-start -space-x-5">
      {currentPost.img.map((img,index) => (
        <>
        <img   className="bg-white border-[#000000] w-[60px] p-2 rounded-md h-[60px] border-2 lg:border-4 hover:z-10 drop-shadow-2xl"   src={`${baseURL}/${img}`} alt="" />
        </> 
      ))}
     </div>
   
        </div>
        <div className="lg:col-span-3 col-span-12  flex justify-center items-center gap-2 flex-col items-center">
...
<path id="Vector" d="M5 16L12 9L19 16" stroke="#ffffff" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
  ...
  <div className="grid grid-cols-12 gap-4 w-[65%]  lg:w-full md:w-full">
    {data.data.map((post, index) => (
      <a href={`/${post.title}/${post.id}`} key={index} className="col-span-12  md:col-span-6 lg:col-span-12 ">
        ...
        <p className="break-words uppercase leading-none text-center font-bold line-clamp-2 w-full">{post.title}</p>
      </a>
    ))}
  </div>
)}
