...
export default function Card({
  ...
  return (
    <div
      className="col-span-12 px-5  md:col-span-12 lg:col-span-4  mb-2"
      key={index}
    >
      <Link to={`/${title}/${id}`} className="w-full h-auto ">
        <div className=" font-patrick text-white bg-transparent px-2 md:px-0 lg:px-0">
          <div className="m-0  rounded-[8px] h-[200px] md:h-[250px] lg:h-[250px] bg-white">
            <img
              src={paths}
          ...
          <div className="p-0 text-white font-patrick">
            <div className="font-patrick  md:mt-1 lg:mt-3 text-sm md:text-[15px] opacity-[60%] w-full break-words flex gap-2">
              <p className="uppercase">{category_name.replace(/ /g, ' | ')}</p>{' '}
              <span>|</span>{' '}
              <p>
